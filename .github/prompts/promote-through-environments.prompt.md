---
description: "Promote changes through environments: dev → staging → preprod → main via pull requests."
---
...
1. **PR before merge**: For each promotion, open a PR *into* the target branch, wait for checks (and review if required), then merge via GitHub.
...
3. Create PR **targeting** the next environment branch:
   ```bash
   gh pr create --base <target> --head <source> --title "…" --body "…"
   ```
4. User or reviewer merges on GitHub when ready.
5. Locally refresh:
   ```bash
   git fetch origin && git checkout <target> && git pull origin <target>
   ```
